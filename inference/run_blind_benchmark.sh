#!/usr/bin/env bash
#
# Blind LLM Benchmark Evaluation Entry Script
#
# Runs lmms-eval with World2Mind tool calling but WITHOUT sending
# original video/image frames to the LLM. The LLM must infer
# categories from the question text and rely solely on cognitive
# map data (YAML + visualization images) for reasoning.
#
# Assumes DA3+SAM3 model service is already running externally
# (started via start_service.py in its own environment).
#
# Usage:
#   bash demo/run_blind_benchmark.sh --gpu_ids 0,1 --tasks vsibench
#   bash demo/run_blind_benchmark.sh --gpu_ids 0,1,2,3 --tasks mindcube_full --model claude-sonnet-4-20250514 --api_type claude
#   bash demo/run_blind_benchmark.sh --gpu_ids 0 --tasks vsibench --limit 10
#   bash demo/run_blind_benchmark.sh --tasks vsibench --resume_log workspace/vsibench/vsibench_blind_claude-opus-4-6_20260310_143052.jsonl
#
...
# ============================================================
# Activate your environment here if needed, e.g.: source /path/to/venv/bin/activate
...
if [[ "$NUM_CONCURRENT" -gt "$MAX_CONCURRENT" ]]; then
    echo "NOTE: Capping NUM_CONCURRENT from ${NUM_CONCURRENT} to ${MAX_CONCURRENT} (workers=${NUM_WORKERS} + 20 buffer)"
    NUM_CONCURRENT=$MAX_CONCURRENT
fi

echo "============================================================"
echo "  Blind LLM Benchmark Evaluation"
echo "  (World2Mind tools enabled, but NO original video/images"
echo "   sent to LLM — cognitive map data only)"
echo "============================================================"
...
# ============================================================
echo ""
echo "[Step 2] Running lmms-eval (blind benchmark mode)..."
...
EVAL_CMD="python -m lmms_eval \
    --model blind_benchmark \
    --model_args ${MODEL_ARGS} \
    --tasks ${TASKS} \
    --batch_size 1 \
    --output_path ${WORKSPACE}/blind_benchmark_results \
    --log_samples"
...
echo ""
echo "============================================================"
echo "  Blind benchmark evaluation complete!"
echo "  Results:  ${WORKSPACE}/blind_benchmark_results"
